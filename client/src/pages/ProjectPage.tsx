...
import {
  ...
  useMotionValue,
  useSpring,
  useTransform as useTransformMR,
  type MotionValue,
} from 'motion/react';
...
const VT_SPRING = { stiffness: 400, damping: 28 };
const VT_THRESHOLD = 100;

const VTChar = React.memo(({ char, mouseX, mouseY }: { char: string; mouseX: MotionValue<number>; mouseY: MotionValue<number>; }) => {
  const ref = useRef<HTMLSpanElement>(null);
  const pos = useRef({ x: 0, y: 0 });
  useEffect(() => {
    const el = ref.current; if (!el) return;
    const sync = () => { pos.current = { x: el.offsetLeft + el.offsetWidth / 2, y: el.offsetTop + el.offsetHeight / 2 }; };
    sync(); window.addEventListener('resize', sync); return () => window.removeEventListener('resize', sync);
  }, []);
  const rawProximity = useTransformMR(mouseX, (mx) => {
    if (mx < -9000) return 0;
    const my = mouseY.get();
    const dist = Math.sqrt((mx - pos.current.x) ** 2 + (my - pos.current.y) ** 2);
    if (dist < VT_THRESHOLD) { const r = 1 - dist / VT_THRESHOLD; return r * r; }
    return Math.max(-1, -((dist - VT_THRESHOLD) / VT_THRESHOLD));
  });
  const proximity = useSpring(rawProximity, VT_SPRING);
  const fontWeight = useTransformMR(proximity, (p) => p >= 0 ? 400 + p * 400 : 400 + p * 150);
  const scale = useTransformMR(proximity, (p) => p >= 0 ? 1 + p * 0.25 : 1 + p * 0.07);
  const y = useTransformMR(proximity, (p) => (p > 0 ? p * -6 : 0));
  const opacity = useTransformMR(proximity, (p) => (p >= 0 ? 1 : 1 + p * 0.55));
  return (
    <mmotion.span ref={ref} style={{ fontWeight, scale, y, opacity, display: 'inline-block', willChange: 'transform, opacity, font-weight' }}>
      {char === ' ' ? '\u00A0' : char}
    </mmotion.span>
  );
});
VTChar.displayName = 'VTChar';

const VariableText = ({ children, as: Tag = 'p', className }: { children: string; as?: 'p' | 'span' | 'h1' | 'h2' | 'h3' | 'h4' | 'h5' | 'h6'; className?: string; }) => {
  const mouseX = useMotionValue(-9999);
  const mouseY = useMotionValue(-9999);
  return (
    <Tag className={`relative cursor-default select-none ${className ?? ''}`}
      onMouseMove={(e: React.MouseEvent<HTMLElement>) => { const rect = e.currentTarget.getBoundingClientRect(); mouseX.set(e.clientX - rect.left); mouseY.set(e.clientY - rect.top); }}
      onMouseLeave={() => { mouseX.set(-9999); mouseY.set(-9999); }}
    >
      {Array.from(children).map((char, i) => <VTChar key={i} char={char} mouseX={mouseX} mouseY={mouseY} />)}
    </Tag>
  );
};

const CAROUSEL_CARD_W = 200; const CAROUSEL_CARD_H = 270; const CAROUSEL_RADIUS = 280;
const DRAG_SENSITIVITY = 0.45; const INERTIA_FRICTION = 0.94; const AUTOSPIN_SPEED = 0.06;
const IDLE_TIMEOUT_MS = 2200; const TILT_SENSITIVITY = 8;

const CarouselCard = React.memo(({ src, transform, w, h }: { src: string; transform: string; w: number; h: number }) => (
  <div className="absolute" style={{ width: w, height: h, transform, transformStyle: 'preserve-3d', willChange: 'transform' }}>
    <div className="w-full h-full overflow-hidden" style={{ borderRadius: '1rem', border: '1px solid rgba(235,89,57,0.15)', boxShadow: '0 8px 40px rgba(0,0,0,0.6)', backfaceVisibility: 'hidden', transition: 'transform 0.3s ease, box-shadow 0.3s ease' }}
      onMouseEnter={e => { const el = e.currentTarget as HTMLDivElement; el.style.boxShadow = '0 16px 60px rgba(0,0,0,0.8), 0 0 0 1px rgba(235,89,57,0.35)'; el.style.transform = 'scale(1.04)'; }}
      onMouseLeave={e => { const el = e.currentTarget as HTMLDivElement; el.style.boxShadow = '0 8px 40px rgba(0,0,0,0.6)'; el.style.transform = 'scale(1)'; }}>
      <img src={src} alt="" width={w} height={h} className="w-full h-full object-cover" loading="lazy" draggable="false" />
    </div>
  </div>
));
...
const ProjectHoverGallery = ({ images }: { images: string[] }) => {
  ...
  const ITEM_W = 11; const ITEM_H = 20; const ACTIVE_W = 44; const HOVER_SCALE = 10; const GAP = 0.8; const PERSPECTIVE = 55; const TRANSITION = 1.25; const GRAYSCALE = 1; const BRIGHTNESS = 0.45;
...
function BackButton() {
  ...
  // Priority: 1) sessionStorage (set by the link that opened this project)
  //           2) document.referrer (full-page navigations from AllProjects)
  //           3) '/' fallback
  ...
  const handleBack = (e: React.MouseEvent) => {
    ...
    // Tell ScrollToTop in App.tsx NOT to reset scroll for this navigation,
    // so the user lands back where they scrolled to (e.g. mid-page Projects section)
  ...
  return (
    ...
    >
      <p
        className="font-button whitespace-nowrap text-[10px] uppercase tracking-[0.18em] text-black"
        style={{
          opacity: hovered ? 1 : 0,
          transform: hovered ? 'translateX(0.75rem)' : 'translateX(0)',
    ...
    const ctx = gsap.context(() => {
      ...
      if (pinnedEl) {
        ...
        ScrollTrigger.create({
          trigger: pinnedEl, start: "top 90%", end: "bottom 10%", scrub: 0.8,
          onUpdate: (self) => {
            if (aboutLabel) {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 mb-24">
            <div className="lg:col-span-4">
              ...
              <div className="editorial-item">
                ...
                <VariableText className="font-button text-sm leading-relaxed text-white/40 max-w-sm">{project.about}</VariableText>
              </div>
            </div>
          ...
          <div className="space-y-0">
            {[{ n: '( 1 )', label: 'Objective', text: project.challenge }, { n: '( 2 )', label: 'Approach', text: project.solution }].map(({ n, label, text }, i) => (
              <div key={i} className="editorial-item group grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-12 py-12 border-t border-white/[0.07] hover:border-primary/30 transition-colors duration-500">
                ...
                <div className="lg:col-span-7 lg:col-start-4"><VariableText className="font-button text-sm leading-relaxed text-white/40 group-hover:text-white/55 transition-colors duration-500">{text}</VariableText></div>
            ...
            >
              ...
              <div className="flex items-end justify-between gap-8">
                <div className="overflow-hidden flex-1">
                  <motion.h3
                    className={`font-white uppercase leading-[0.82] tracking-tight ${SCROLL_MAIN} group-hover:text-primary transition-colors duration-700 block`}
                    style={{ fontSize: 'clamp(3.5rem, 9vw, 11rem)' }}
                    data-cursor="big"
                    whileHover={{ x: 12 }}
                    transition={{ type: "spring", stiffness: 300, damping: 30 }}
                  >{nextProject.title}</motion.h3>
                </div>

                {/* ─── TextReveal Button — si apre quando l'intera sezione è hovered ─── */}
                <div className="hidden md:flex flex-col items-end gap-3 pb-3 shrink-0">
                  <div
                    className="relative inline-flex h-12 items-center justify-center overflow-hidden rounded-full bg-[hsl(11,80%,57%)] transition-all duration-300"
                    style={{ width: nextHovered ? '11rem' : '3rem' }}
                  >
                    <p
                      className="font-button inline-flex whitespace-nowrap text-[10px] uppercase tracking-[0.18em] text-black transition-all duration-200"
                      style={{ opacity: nextHovered ? 1 : 0, transform: nextHovered ? 'translateX(-0.75rem)' : 'translateX(0)' }}
                    >
                      Next Project
                    </p>
