...
// ─── FocusFrame (inline, same logic as TrueFocus) ────────────────────────────
    ...
    >
      {([
        ...
        <span key={k} style={{
          ...
          filter: `drop-shadow(0px 0px 5px ${GLOW_COLOR})`,
          borderRadius: "2px", ...s,
        }} />
      ))}
    </motion.div>
...
// ─── StoryTitle — "The" + parola fissa, inline, frame alterna tra i due ──────
...
function StoryTitle({
  ...
  const [focusOnThe, setFocusOnThe] = useState(true);
  ...
  return (
    <span ref={wrapRef} className="relative inline-flex items-baseline gap-2 mb-8" data-cursor="big">
      <span
        ref={theRef}
        className={`whitespace-nowrap tracking-tight uppercase font-extrabold leading-none ${focusOnThe ? "font-orange" : "font-white"}`}
        style={wordStyle(focusOnThe)}
      >
        The
      </span>
      <span
...
const Char = React.memo(({
  ...
  const rawProximity = useTransform(mouseX, (mx) => {
    ...
    if (dist < VT_THRESHOLD) {
      const r = 1 - dist / VT_THRESHOLD;
      return r * r;
    }
  ...
  const fontWeight = useTransform(proximity, (p) => p >= 0 ? 400 + p * 400 : 400 + p * 150);
  const scale      = useTransform(proximity, (p) => p >= 0 ? 1 + p * 0.25 : 1 + p * 0.07);
  const y          = useTransform(proximity, (p) => (p > 0 ? p * -6 : 0));
  const opacity    = useTransform(proximity, (p) => (p >= 0 ? 1 : 1 + p * 0.55));
...
const VariableText = ({
  ...
  return (
    <p
      className={`relative cursor-default select-none font-button text-sm font-light leading-relaxed text-white/45 ${className ?? ""}`}
      onMouseMove={(e: React.MouseEvent<HTMLParagraphElement>) => {
...
export const PinnedStoryReveal: React.FC<Props> = ({ project, projectIndex }) => {
    ...
    const base = [
      {
        ...
        image: project.extraMedia[0],
      },
      {
        index: "02",
        tag: "Fix",
        body: project.solution,
        image: project.extraMedia[1],
      },
    ];
  ...
  // ─── GSAP: pin right column + mask reveal ────────────────────────────────
  useEffect(() => {
    ...
    >
      <div className="relative z-10 flex gap-0 justify-between max-w-[1200px] mx-auto px-6 lg:px-16">
        ...
        <div className="psrv__left flex flex-col min-w-0 flex-1 pr-8 lg:pr-16">
          ...
          {/* Section label */}
          <div
            className="sticky top-0 z-20 pt-8 pb-4 flex items-center gap-4"
            style={{ backgroundColor: "#000000" }}
          >
            <span className="font-button text-[9px] uppercase tracking-[0.55em] text-white/30">
              03 — Story
            </span>
          ...
          {panels.map((panel, i) => (
            <div
              ...
              style={{ minHeight: "100vh", paddingBlock: "8rem" }}
            >
              <span
                className="font-button text-[9px] uppercase tracking-[0.55em] block mb-6"
                style={{ color: PRIMARY }}
              >
                {panel.index} — {panel.tag}
              </span>

              {/* ── Titolo: "The" + keyword fisso, inline, frame TrueFocus ── */}
              <StoryTitle
                keyword={panel.tag}
                fontSize="clamp(1.9rem, 3vw, 3.4rem)"
                focusPause={3000}
              ...
              {/* VariableText — char-level hover effect */}
              <VariableText>{panel.body}</VariableText>

              {/* Results list in last panel */}
              {i === panels.length - 1 && project.results.length > 0 && (
                <div className="mt-10 space-y-0">
                  {project.results.map((result, ri) => (
                    <div
                      key={ri}
                      className="group flex gap-5 items-center py-6 border-t border-white/[0.08] last:border-b
                                 hover:bg-white/[0.03] hover:px-4 rounded-xl transition-all duration-500 -mx-4 px-4"
                    >
                      <span className="font-button text-xs min-w-[2rem] shrink-0" style={{ color: PRIMARY }}>
                        {String(ri + 1).padStart(2, "0")}
                      </span>
                      <p
                        className={`font-white uppercase flex-1 leading-tight tracking-tight ${SCROLL_MAIN}`}
                      ...
                      <div
                        className="w-7 h-7 rounded-full border border-white/10 flex items-center justify-center
                                   group-hover:border-primary group-hover:bg-primary group-hover:text-black
                                   transition-all duration-300 shrink-0 text-white/40"
                      >
                        <ArrowRight size={11} />
                      </div>
                    </div>
              ...
              <div
                className="mt-16 font-white uppercase leading-none select-none pointer-events-none opacity-[0.04]"
        ...
        >
          {panels.map((panel, i) => (
            ...
            >
              <div
                className="absolute top-0 left-0 w-14 h-[2px] z-10 pointer-events-none"
                style={{ background: `linear-gradient(to right, ${PRIMARY}, transparent)` }}
              />
              <div
                className="absolute top-0 left-0 w-[2px] h-14 z-10 pointer-events-none"
