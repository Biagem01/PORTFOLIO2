    ...
    <>
      {words.map((w, idx) => {
        ...
        return (
          <span
            key={`${w}-${idx}`}
            data-cursor="hide"   // ✅ FIX DEFINITIVO
            className={
              isHot
                ? "text-[rgb(var(--hero-orange))] font-semibold"
                : "text-[rgb(var(--hero-gold))]/85 font-medium"
            }
          >
            {w}
            {idx < words.length - 1 ? " " : ""}
...
function ProjectPlayReelSlide({
  ...
  return (
    <motion.div
      ...
      // renderizza comunque l'opacità animata, ma abilita pointer events solo sulla slide attiva
      style={{
        opacity: slideOpacity,
        // rendi cliccabile solo la slide attiva
        pointerEvents: activeIndex === index ? "auto" : "none",
        // opzionale: assicurati che la slide attiva stia sopra alle altre
    ...
    >
      ...
      >
        {/* ✅ VIEW cursor SOLO sul reel */}
        {/* ----- SPOSTO QUI: BUTTON VIEW (copre tutto, sempre sopra) ----- */}
        <motion.button
          type="button"
          aria-label={`View details for ${project.title}`}
          onClick={goToProject}
          data-cursor="view"
          // z molto alto per sovrastare eventuali stacking context
        ...
        {/* ✅ TEXT OVERLAY sopra al button del reel */}
        <motion.div
          data-cursor="hide"
          className="absolute inset-0 z-[70] pointer-events-none"
          style={{ opacity: textOpacity }}
        >
          {/* NUMBER (hide, non cliccabile) */}
          <motion.span
            data-cursor="hide"
          ...
          >
            ...
            <div className="mt-4 ml-auto max-w-[34rem] pointer-events-none">
              ...
              >
                <p
                  className="
                    text-right
                    text-[0.95rem] md:text-[1.02rem] lg:text-[1.08rem]
                    leading-[1.55] tracking-[-0.01em]
                    italic font-medium
                    text-[rgb(var(--hero-gold))]/86
            ...
            {/* ✅ CTA: unica cosa cliccabile nel testo, cursor hide */}
            <button
        type="button"
        onClick={goToProject}
        data-cursor="hide"
