...
const About = () => {
  // ✅ Titoli / UI: scroll-indicator
  const SCROLL_MAIN = "text-[hsl(var(--scroll-indicator))]";
  const LABEL_MUTED = "text-foreground/50";

  // ✅ Body text Minh: rgb(183,171,152)
  // (usa --body-text se l’hai aggiunta in index.css, altrimenti puoi lasciare rgb(...) diretto)
  const BODY =
    "text-xs md:text-sm leading-relaxed uppercase antialiased font-bold text-[rgb(var(--body-text))]";

  // ✅ Descrizioni card: stesso beige body
  const CARD_DESC =
    "text-xs md:text-sm leading-relaxed antialiased font-bold text-[rgb(var(--body-text))]";
      ...
      <div className="max-w-6xl mx-auto grid md:grid-cols-[0.85fr,1.15fr] gap-12 lg:gap-20">
        ...
        <div className="space-y-16 md:space-y-20">
          ...
          <motion.div
            ...
            className="space-y-5"
          >
            <p className={`${BODY}  `} data-cursor="big" >
              Sono uno sviluppatore front-end con attenzione al dettaglio. Mi piace
              costruire prodotti chiari, ordinati e piacevoli da usare.
            </p>

            <p className={BODY} data-cursor="big">
              Quando progetto una pagina non penso solo a come{" "}
              <span className="uppercase font-medium text-[hsl(var(--accent-orange))]">
                appare
              </span>
              , ma anche a come{" "}
              <span className="uppercase font-medium text-[hsl(var(--accent-orange))]">
            ...
            <p className={BODY} data-cursor="big">
              ...
              : niente elementi inutili, solo ciò che serve per comunicare un’esperienza
              pulita e coerente.
            </p>
          </motion.div>
              ...
              {services.map((service, index) => (
                ...
                >
                  <div className="relative z-10 space-y-2.5">
                    <h4 className="text-base md:text-lg text-[hsl(var(--accent-orange))] tracking-tight">
                      {service.title}
                    </h4>

                    {/* ✅ FIX: non più scroll-indicator qui */}
