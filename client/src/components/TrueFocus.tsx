...
/**
 * HeroBadgesFocus — 4 parole fisse, focus alterna tra le coppie:
 *   Stato A: word0 + word2 arancione  →  word1 + word3 blur
 *   Stato B: word1 + word3 arancione  →  word0 + word2 blur
 */
...
export function HeroBadgesFocus({
  ...
  useEffect(() => {
    const update = () => {
      ...
      if (elTop) {
        const r = elTop.getBoundingClientRect();
        setRect0({ x: r.left - a.left, y: r.top - a.top, width: r.width, height: r.height });
      }
      if (elBot) {
        const r = elBot.getBoundingClientRect();
        setRect1({ x: r.left - a.left, y: r.top - a.top, width: r.width, height: r.height });
      }
      setReady(true);
    };
    update();
    ...
    // Ricalcola quando il componente rientra nel viewport
    const observer = new IntersectionObserver(
      (entries) => { if (entries[0].isIntersecting) { update(); setTimeout(update, 80); } },
      { threshold: 0.01 }
    );
    if (frameAnchorRef.current) observer.observe(frameAnchorRef.current);
  ...
  const wordStyle = (isActive: boolean) => ({
  ...
  return (
    <>
      <span className="block">
        <span ref={ref0} className={`whitespace-nowrap tracking-tight uppercase font-extrabold leading-none ${focusOnA ? 'font-orange' : 'font-white'}`} style={wordStyle(focusOnA)}>
          {word0}
        </span>
      </span>

      <span className="block">
        <span ref={ref1} className={`whitespace-nowrap tracking-tight uppercase font-extrabold leading-none ${!focusOnA ? 'font-orange' : 'font-white'}`} style={wordStyle(!focusOnA)}>
          {word1}
        </span>
      </span>

      <span className="block">
        <span ref={ref2} className={`whitespace-nowrap tracking-tight uppercase font-extrabold leading-none ${focusOnA ? 'font-orange' : 'font-white'}`} style={wordStyle(focusOnA)}>
          {word2}
        </span>
      </span>

      <span className="block">
        <span ref={ref3} className={`whitespace-nowrap tracking-tight uppercase font-extrabold leading-none ${!focusOnA ? 'font-orange' : 'font-white'}`} style={wordStyle(!focusOnA)}>
          {word3}
        </span>
      </span>
...
// ═══════════════════════════════════════════════════════════════════════════════
// TwoWordFocus — alterna il frame TrueFocus tra sole 2 parole verticali
//   Stato A: word0 arancione  word1 blur
//   Stato B: word0 blur       word1 arancione
// ═══════════════════════════════════════════════════════════════════════════════
  ...
  return (
    <>
      <span className="block">
        <span
          ref={ref0}
          className={`whitespace-nowrap tracking-tight uppercase font-extrabold leading-none ${focusOnA ? 'font-orange' : 'font-white'}`}
          style={wordStyle(focusOnA)}
        >
          {word0}
        </span>
      </span>
      ...
      {ready && (
        <FocusFrame rect={rect} dur={dur} borderColor={borderColor} glowColor={glowColor} />
      )}
    </>
  );
}
